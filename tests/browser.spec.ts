...
test('garden brightness persists, follows the device when asked, and does not interrupt play', async ({ page }) => {
  await page.goto('/');
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'light');
  await page.getByRole('button', { name: 'Switch to dusk garden' }).click();
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'dark');
  expect(await page.evaluate(() => localStorage.getItem('worm-theme'))).toBe('dark');
  await page.screenshot({ path: 'test-results/home-dark.png', fullPage: true });
  const soil = await page.locator('.garden').evaluate(el => getComputedStyle(el).backgroundColor);
  await page.reload();
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'dark');
  await expect(page.getByRole('button', { name: 'Switch to ivory garden' })).toBeVisible();
  await page.getByRole('button', { name: 'Settings', exact: true }).click();
  ...
  await page.getByRole('button', { name: 'Switch to dusk garden' }).click();
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'dark');
  await expect(page.locator('.game-page')).toHaveAttribute('data-phase', phase!);
  await page.screenshot({ path: 'test-results/game-dark.png' });
  expect(await page.locator('.garden').evaluate(el => getComputedStyle(el).backgroundColor)).toBe(soil);
  expect(await readSession(page, id)).toBeTruthy();
});
